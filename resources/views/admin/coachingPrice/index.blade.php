    ...
    <script>
        ...
        $(document).on('dblclick','#coachingprice-table #table-data tr',function (e) {
            ...
            $.get("{{ Auth::user()->name }}",{id:hidden_id},function (data) {
                ...
                $('#coachingprice-form-edit input[name="name"]').val(data.name);
                $('#coachingprice-form-edit select[name="league_id"] option[value="'+data.league_id+'"]').prop('selected', true);
                if(data.p_name != null)
                {
                    $('#coachingprice-form-edit img[name="p_name"]').attr('src','{{ URL::to('images') .'/' }}' + data.p_name);
                }
                else
                {
                    $('#coachingprice-form-edit img[name="p_name"]').attr('src','{{ URL::to('images') .'/' }}400x400.png');
                }
